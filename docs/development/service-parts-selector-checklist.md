...
## Phase 3: Frontend UI Components (Vanilla JS / jQuery)
- [x] Setup `public/js/service_parts_selector.bundle.js` build configuration.
- [x] Create class `ServicePartsSelectorDialog` for the Dialog/Modal interface.
...
- [x] Inject `service_parts_selector.bundle.js` into targeted DocTypes (`Quotation`, `Sales Order`, `Sales Invoice`, `Purchase Receipt`, `Stock Entry`) via `doctype_js` in `hooks.py`.
- [x] Add a custom button `"Service & Parts Selector"` to the items table in targeted forms (using `frm.add_custom_button`).
- [x] Implement logic to extract the `project` reference from the active document and pass it to the selector/backend for vehicle context filtering.
- [x] Implement Item Insertion Logic (JavaScript) to push selections back to the ERPNext document:
